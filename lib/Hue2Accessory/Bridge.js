  ...
  async analyseFullState (fullState) {
    ...
    for (const rid in this.resourceByRidByRtype.device ?? {}) {
      ...
      if (
        ...
        activeAccessoryIds.add(device.serialNumber)
    ...
    for (const scene of Object.values(this.resourceByRidByRtype.scene ?? {})) {
      const groupId = scene.group?.rid
      if (groupId != null) {
        if (scenesByGroupId[groupId] == null) {
          scenesByGroupId[groupId] = []
        }
        scenesByGroupId[groupId].push(scene)
      }
    }
    for (const type of ['room', 'zone']) {
      for (const group of Object.values(this.resourceByRidByRtype[type] ?? {})) {
        const groupedLightService = group.services?.find((svc) => {
          return (svc.rtype ?? svc.type) === 'grouped_light'
        })
        const groupedLight = groupedLightService == null
          ? null
          : this.resourceByRidByRtype.grouped_light?.[groupedLightService.rid]
        if (groupedLight != null) {
          const id = `${group.type}:${group.id}`
          const scenes = scenesByGroupId[group.id] ?? []
          activeAccessoryIds.add(id)
          if (this.accessoryById[id] == null) {
            this.accessoryById[id] = new Hue2Accessory.Group(
              this, group, groupedLight, scenes
            )
          } else {
            ...
            this.accessoryById[id].syncScenes(scenes)
          }
        }
      }
    }
    for (const id of Object.keys(this.accessoryById)) {
      if (!activeAccessoryIds.has(id)) {
        this.deleteAccessory(id)
      }
    }
