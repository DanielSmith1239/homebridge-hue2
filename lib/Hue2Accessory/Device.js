...
function accessoryName (device) {
  const product = productData(device)
  return device.metadata?.name ?? product.product_name ?? device.id
}

function accessoryManufacturer (device) {
  const product = productData(device)
  return product.manufacturer_name ?? 'Signify Netherlands B.V.'
}

function accessoryModel (device) {
  const product = productData(device)
  return product.model_id ?? product.product_name ?? device.type
}

function accessoryFirmware (device, bridge) {
  const product = productData(device)
  return product.software_version ?? bridge.values.software
}
